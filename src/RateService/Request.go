...
var _PackageProductionUrl = "https://onlinetools.ups.com/webservices/Rate"
var _PackageTestUrl = "https://wwwcie.ups.com/webservices/Rate"

func NewRatePortType(security *UPSSecurity, testEnv bool) RatePortType {
	var client = new(soap.Client)
	if testEnv {
		client = soap.NewClient(_PackageTestUrl)
	}else {
		client = soap.NewClient(_PackageProductionUrl)
	}
